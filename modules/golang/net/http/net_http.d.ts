...
declare module 'golang/net/http'{
	...
	// @ts-ignore
	import type {Struct,bool,Alias,map,Ref,error,int,GoError,int64,Nothing} from 'go'
	...
	export interface Client extends Struct<Client>{
			...
			checkRedirect:(req:Ref<Request>,via:Ref<Request>[])=>void/*error*/
			jar:CookieJar
			timeout:time.Duration
			get(url:string):Ref<Response>
			Do(req:Ref<Request>):Ref<Response>
			post(url:string,contentType:string,body:io.Reader):Ref<Response>
			postForm(url:string,data:url.Values):Ref<Response>
			head(url:string):Ref<Response>
	...
	export interface File extends io.Reader,io.Seeker,io.Closer{

			readdir(count:int):fs.FileInfo[]
			stat():fs.FileInfo
	}
	...
	export function handleFunc(pattern:string,v1:(v2:ResponseWriter,v1:Ref<Request>)=>void):void
	...
	export interface Hijacker{
			...
			hijack():[net.Conn,Ref<bufio.ReadWriter>]
	}
	...
	export interface MaxBytesError extends Error,GoError,Struct<MaxBytesError>{
	...
	export function newRequest(method:string,url:string,body:io.Reader):Ref<Request>

	export function newRequestWithContext(ctx:context.Context,method:string,url:string,body:io.Reader):Ref<Request>
	...
	export function post(url:string,contentType:string,body:io.Reader):Ref<Response>

	export function postForm(url:string,data:url.Values):Ref<Response>
	...
	export function readResponse(r:Ref<bufio.Reader>,req:Ref<Request>):Ref<Response>
	...
	export interface Request extends Struct<Request>{
			...
			userAgent():string
			cookies():Ref<Cookie>[]
			cookie(name:string):Ref<Cookie>
			addCookie(c:Ref<Cookie>):void
			referer():string
			multipartReader():Ref<multipart.Reader>
			write(w:io.Writer)/*error*/
			...
			formFile(key:string):[multipart.File,Ref<multipart.FileHeader>]
	}
	export interface Response extends Struct<Response>{
			...
			tls:Ref<tls.ConnectionState>
			cookies():Ref<Cookie>[]
			location():Ref<url.URL>
			protoAtLeast(major:int,minor:int):bool
			write(w:io.Writer)/*error*/
	}
	export interface ResponseController extends Hijacker,Struct<ResponseController>{
	...
	export interface ResponseWriter{
			...
			header():Header
			write(v1:Uint8Array):int
			writeHeader(statusCode:int):void
	}
	export interface RoundTripper{

			roundTrip(v1:Ref<Request>):Ref<Response>
	}
	...
	export interface ServeMux extends Struct<ServeMux>,Handler{

			handler(r:Ref<Request>):[Handler,string]
			serveHTTP(w:ResponseWriter,r:Ref<Request>):void
			handle(pattern:string,handler:Handler):void
			handleFunc(pattern:string,v1:(v2:ResponseWriter,v1:Ref<Request>)=>void):void
	}
	...
	export interface Server extends Struct<Server>,io.Closer{
			...
			maxHeaderBytes:int
			tlsNextProto:map<string,(v3:Ref<Server>,v2:Ref<tls.Conn>,v1:Handler)=>void>
	...
	export interface Transport extends Struct<Transport>,RoundTripper{
			...
			proxy:(v1:Ref<Request>)=>Ref<url.URL>
			onProxyConnectResponse:(ctx:context.Context,proxyURL:Ref<url.URL>,connectReq:Ref<Request>,connectRes:Ref<Response>)=>void/*error*/
			dialContext:(ctx:context.Context,network:string,addr:string)=>net.Conn
			dial:(network:string,addr:string)=>net.Conn
			dialTLSContext:(ctx:context.Context,network:string,addr:string)=>net.Conn
			dialTLS:(network:string,addr:string)=>net.Conn
			...
			tlsNextProto:map<string,(authority:string,c:Ref<tls.Conn>)=>RoundTripper>
			proxyConnectHeader:Header
			getProxyConnectHeader:(ctx:context.Context,proxyURL:Ref<url.URL>,target:string)=>Header
			maxResponseHeaderBytes:int64
			writeBufferSize:int
			readBufferSize:int
			forceAttemptHTTP2:bool
			roundTrip(req:Ref<Request>):Ref<Response>
			clone():Ref<Transport>
	...
	export function emptyRefServer():Ref<Server>
	export function refOfServer(x:Server,v:Ref<Server>)
	export function unRefServer(v:Ref<Server>):Server
	export function emptyTransport():Transport
	export function emptyRefTransport():Ref<Transport>
	export function refOfTransport(x:Transport,v:Ref<Transport>)
	export function unRefTransport(v:Ref<Transport>):Transport
	export function emptyPushOptions():PushOptions
	export function emptyRefPushOptions():Ref<PushOptions>
	export function refOfPushOptions(x:PushOptions,v:Ref<PushOptions>)
	export function unRefPushOptions(v:Ref<PushOptions>):PushOptions
	...
	export function emptyRefCookie():Ref<Cookie>
	export function refOfCookie(x:Cookie,v:Ref<Cookie>)
	export function unRefCookie(v:Ref<Cookie>):Cookie
}