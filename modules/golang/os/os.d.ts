...
declare module 'golang/os'{
	...
	// @ts-ignore
	import type {error,int,Ref,Struct,int64,uintptr,bool,rune,GoError,uint8} from 'go'
	...
	export function createTemp(dir:string,pattern:string):Ref<File>
	...
	export function expand(s:string,v1:(v1:string)=>string):string

	export function expandEnv(s:string):string

	export interface File extends fs.ReadDirFile,io.ReaderAt,io.ReadWriteCloser,io.Closer,io.WriterAt,io.WriteCloser,io.StringWriter,io.WriteSeeker,syscall.Conn,io.ReadSeekCloser,io.ReaderFrom,io.ReadWriter,io.ReadCloser,io.ReadSeeker,Struct<File>,io.ReadWriteSeeker{

			readdir(n:int):fs.FileInfo[]
			readdirnames(n:int):string[]
			readDir(n:int):fs.DirEntry[]
			name():string
			read(b:Uint8Array):int
			readAt(b:Uint8Array,off:int64):int
			readFrom(r:io.Reader):int64
			write(b:Uint8Array):int
			writeAt(b:Uint8Array,off:int64):int
	...
	export interface LinkError extends Error,Struct<LinkError>,GoError{
	...
	export function openFile(name:string,flag:int,perm:fs.FileMode):Ref<File>
	...
	export function pipe():[Ref<File>,Ref<File>]
	...
	export function startProcess(name:string,argv:string[],attr:Ref<ProcAttr>):Ref<Process>
	...
	export interface SyscallError extends Struct<SyscallError>,GoError,Error{
	...
	export function emptyRefProcessState():Ref<ProcessState>
	export function refOfProcessState(x:ProcessState,v:Ref<ProcessState>)
	export function unRefProcessState(v:Ref<ProcessState>):ProcessState
}