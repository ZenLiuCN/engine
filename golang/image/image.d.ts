...
declare module 'golang/image'{
	...
	// @ts-ignore
	import type {bool,Ref,error,GoError,uint8,uint32,int,Struct} from 'go'
	...
	export interface Rectangle extends RGBA64Image,Struct<Rectangle>{
		min:Point
		max:Point
		string():string
		dx():int
	...
	export function registerFormat(name:string,magic:string,decode:(v1:io.Reader)=>Image,decodeConfig:(v1:io.Reader)=>Config):void
	export const Transparent:Ref<Uniform>
	export interface Uniform extends Struct<Uniform>,color.Color,color.Model,RGBA64Image{
		C:color.Color
