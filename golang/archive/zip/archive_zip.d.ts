...
declare module 'golang/archive/zip'{
	...
	// @ts-ignore
	import type {uint32,Ref,Alias,uint16,GoError,int64,error,Struct,bool,uint64} from 'go'
	...
	export function newReader(r:io.ReaderAt,size:int64):[Ref<Reader>,error]
	export function newWriter(w:io.Writer):Ref<Writer>
	export function openReader(name:string):Ref<ReadCloser>
	export interface ReadCloser extends io.Closer,Struct<ReadCloser>,fs.FS{
		reader:Reader
		close():error
	}
	...
	export interface Writer extends Struct<Writer>,io.Closer{
		...
		createHeader(fh:Ref<FileHeader>):[io.Writer,error]
		createRaw(fh:Ref<FileHeader>):[io.Writer,error]
