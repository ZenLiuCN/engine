...
declare module 'golang/crypto/rsa'{
	...
	// @ts-ignore
	import type {Struct,error,GoError,int,bool,Ref} from 'go'
	export interface CRTValue extends Struct<CRTValue>,crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey{
		exp:Ref<big.Int>
		coeff:Ref<big.Int>
		R:Ref<big.Int>
	}
	...
	export interface OAEPOptions extends Struct<OAEPOptions>,crypto.DecrypterOpts,crypto.PublicKey,crypto.PrivateKey{
		hash:crypto.Hash
		mgfHash:crypto.Hash
		label:Uint8Array
	}
	export interface PKCS1v15DecryptOptions extends crypto.PrivateKey,Struct<PKCS1v15DecryptOptions>,crypto.DecrypterOpts,crypto.PublicKey{
		sessionKeyLen:int
	}
	export interface PSSOptions extends Struct<PSSOptions>,crypto.DecrypterOpts,crypto.PublicKey,crypto.PrivateKey,crypto.SignerOpts{
		saltLength:int
		hash:crypto.Hash
	...
	export interface PrecomputedValues extends Struct<PrecomputedValues>,crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey{
		dp:Ref<big.Int>
		dq:Ref<big.Int>
		qinv:Ref<big.Int>
		crtValues:CRTValue[]
	}
	export interface PrivateKey extends Struct<PrivateKey>,crypto.DecrypterOpts,crypto.PublicKey,crypto.PrivateKey,crypto.Signer,crypto.Decrypter{
		publicKey:PublicKey
		D:Ref<big.Int>
		primes:Ref<big.Int>[]
