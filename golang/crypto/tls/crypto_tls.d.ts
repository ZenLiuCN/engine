...
declare module 'golang/crypto/tls'{
	...
	export interface Certificate extends Struct<Certificate>,crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey{
	...
	export interface CertificateRequestInfo extends crypto.PrivateKey,Struct<CertificateRequestInfo>,crypto.PublicKey,crypto.DecrypterOpts{
	...
	export interface CertificateVerificationError extends Error,GoError,Struct<CertificateVerificationError>{
		unverifiedCertificates:Ref<x509.Certificate>[]
		err:GoError
		error():string
		unwrap():error
	}
	export interface CipherSuite extends crypto.DecrypterOpts,crypto.PrivateKey,Struct<CipherSuite>,crypto.PublicKey{
		id:uint16
		name:string
	...
	export interface ClientHelloInfo extends crypto.PublicKey,Struct<ClientHelloInfo>,crypto.DecrypterOpts,crypto.PrivateKey{
	...
	export interface ClientSessionState extends Struct<ClientSessionState>,crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey{
		resumptionState():[Uint8Array,Ref<SessionState>,error]
	}
	export interface Config extends crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey,Struct<Config>{
		rand:io.Reader
		time:()=>time.Time
		certificates:Certificate[]
		nameToCertificate:Record<string,Ref<Certificate>>
		getCertificate:(v1:Ref<ClientHelloInfo>)=>[Ref<Certificate>,error]
		getClientCertificate:(v1:Ref<CertificateRequestInfo>)=>[Ref<Certificate>,error]
		getConfigForClient:(v1:Ref<ClientHelloInfo>)=>[Ref<Config>,error]
		verifyPeerCertificate:(rawCerts:Array<Uint8Array>,verifiedChains:Ref<x509.Certificate>[][])=>error
		verifyConnection:(v1:ConnectionState)=>error
		...
		unwrapSession:(identity:Uint8Array,cs:ConnectionState)=>[Ref<SessionState>,error]
		wrapSession:(v2:ConnectionState,v1:Ref<SessionState>)=>[Uint8Array,error]
		minVersion:uint16
		maxVersion:uint16
		...
		encryptTicket(cs:ConnectionState,ss:Ref<SessionState>):[Uint8Array,error]
		decryptTicket(identity:Uint8Array,cs:ConnectionState):[Ref<SessionState>,error]
	}
	export interface Conn extends io.ReadWriteCloser,crypto.PublicKey,net.Conn,crypto.DecrypterOpts,io.ReadCloser,crypto.PrivateKey,io.Closer,Struct<Conn>,io.ReadWriter,io.WriteCloser{
		localAddr():net.Addr
		remoteAddr():net.Addr
	...
	export interface ConnectionState extends crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey,Struct<ConnectionState>{
		version:uint16
		...
		exportKeyingMaterial(label:string,context:Uint8Array,length:int):[Uint8Array,error]
	}
	...
	export function dial(network:string,addr:string,config:Ref<Config>):[Ref<Conn>,error]
	export function dialWithDialer(dialer:Ref<net.Dialer>,network:string,addr:string,config:Ref<Config>):[Ref<Conn>,error]
	export interface Dialer extends Struct<Dialer>,crypto.PublicKey,crypto.DecrypterOpts,crypto.PrivateKey{
	...
	export function listen(network:string,laddr:string,config:Ref<Config>):[net.Listener,error]
	export function loadX509KeyPair(certFile:string,keyFile:string):[Certificate,error]
	...
	export function newResumptionState(ticket:Uint8Array,state:Ref<SessionState>):[Ref<ClientSessionState>,error]
	...
	export interface QUICConfig extends Struct<QUICConfig>,crypto.DecrypterOpts,crypto.PrivateKey,crypto.PublicKey{
		tlsConfig:Ref<Config>
	}
	export interface QUICConn extends crypto.PrivateKey,io.Closer,Struct<QUICConn>,crypto.PublicKey,crypto.DecrypterOpts{
	...
	export interface QUICEvent extends Struct<QUICEvent>,crypto.PrivateKey,crypto.PublicKey,crypto.DecrypterOpts{
		kind:QUICEventKind
	...
	export interface QUICSessionTicketOptions extends crypto.PublicKey,Struct<QUICSessionTicketOptions>,crypto.DecrypterOpts,crypto.PrivateKey{
		earlyData:bool
	}
	...
	export function x509KeyPair(certPEMBlock:Uint8Array,keyPEMBlock:Uint8Array):[Certificate,error]
