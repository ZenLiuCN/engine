...
declare module 'golang/log/slog'{
	...
	// @ts-ignore
	import type {Struct,int64,uintptr,int,uint64,bool,Ref,float64,error} from 'go'
	export function any(key:string,value:any):Attr
	export function anyValue(v:any):Value
	export interface Attr extends json.Token,Struct<Attr>,fmt.Stringer{
		key:string
		value:Value
		equal(b:Attr):bool
		string():string
	...
	export interface LevelVar extends Leveler,encoding.TextUnmarshaler,encoding.TextMarshaler,fmt.Stringer,Struct<LevelVar>,json.Token{
		level():Level
		set(l:Level):void
		string():string
		marshalText():[Uint8Array,error]
	...
	export interface TextHandler extends json.Token,Struct<TextHandler>,Handler{
		enabled(_:context.Context,level:Level):bool
		withAttrs(attrs:Attr[]):Handler
		withGroup(name:string):Handler
		handle(_:context.Context,r:Record):error
