...
declare module 'golang/net/rpc'{
	...
	// @ts-ignore
	import type {Struct,error,uint64,GoError,Ref} from 'go'
	...
	export interface Client extends io.Closer,Struct<Client>{
		close():error
	...
	export function dial(network:string,address:string):[Ref<Client>,error]
	export function dialHTTP(network:string,address:string):[Ref<Client>,error]
	export function dialHTTPPath(network:string,address:string,path:string):[Ref<Client>,error]
