...
func TestSourceMapLoading(t *testing.T) {
	e := Get()
	code, src := compileSource("sample.js",
		//language=typescript
		`
const err=()=>{
throw Error("Just Fail")
}
err()
`, true)
	n := e.registerMapping(NewSourceMap(src), src)
	_, err := e.Runtime.RunScript("sample.js", code+n)
	if err != nil {
		panic(e.parse(err))
	}
}
func TestSourceMapLoading2(t *testing.T) {
	e := Get()
	code, src := compileSource("sample.js",
		//language=typescript
...
func compileSource(file, ts string, entry bool) (string, []byte) {
